...
namespace EliteMMO.Scripted.Views
{
    public partial class ScriptNaviMap : UserControl
    {
        ...
        private void BgwNaviDoWork(object sender, DoWorkEventArgs e)
        {
            var count = 0;
            float dir = -90;
            while (isRunning || !bgw_navi.CancellationPending)
            {
                if (isRecording)
                {
                    ...
                    var distance = Math.Sqrt(Math.Pow(Math.Abs(ScriptFarmDNC.PlayerInfo.X - float.Parse(items[1])), 2) +
                                             Math.Pow(Math.Abs(ScriptFarmDNC.PlayerInfo.Y - float.Parse(items[3])), 2) +
                                             Math.Pow(Math.Abs(ScriptFarmDNC.PlayerInfo.Z - float.Parse(items[2])), 2));
                    if (distance > (double) NodeDist.Value)
                    {
                        if (ScriptFarmDNC.PlayerInfo.X == 0 && ScriptFarmDNC.PlayerInfo.Y == 0 &&
                            ScriptFarmDNC.PlayerInfo.Z == 0)
                        {
                            WayPoints.Items.Add(
                                $"WAYPOINT:{ScriptFarmDNC.PlayerInfo.X}:{ScriptFarmDNC.PlayerInfo.Z}:{ScriptFarmDNC.PlayerInfo.Y}:Zoning;{lastH}");
                        }
                        else
                        {
                            lastH = ScriptFarmDNC.PlayerInfo.H;
                            WayPoints.Items.Add(
                                $"WAYPOINT:{ScriptFarmDNC.PlayerInfo.X}:{ScriptFarmDNC.PlayerInfo.Z}:{ScriptFarmDNC.PlayerInfo.Y}");
                        }
                    }
                }
                ...
                if (isPlaying)
                {
                    if (comboBox2.Text != "" && ScriptFarmDNC.PlayerInfo.Status == 0 && !isPaused)
                    {
                        ...
                        if (firstPersonView.Checked || navPathfirst[closestWayPoint])
                        {
                            ...
                            api.Entity.GetLocalPlayer().H = (float) (Math.PI / 180 *
                                                                     (ScriptFarmDNC.PlayerInfo.GetAngleFrom(
                                                                         navPathX[closestWayPoint],
                        ...
                        api.AutoFollow.SetAutoFollowCoords(
                            (float) navPathX[closestWayPoint] - ScriptFarmDNC.PlayerInfo.X,
                            navPathY[closestWayPoint] == 0
                                ? 0
                                : (float) navPathY[closestWayPoint] - ScriptFarmDNC.PlayerInfo.Y,
                        ...
                        //}
                        if (navPathdoor[closestWayPoint].Contains("Door")) CheckDoor(closestWayPoint);
                    ...
                    if (StuckWatch.Checked && comboBox2.Text != "" && api.AutoFollow.IsAutoFollowing && !isPaused &&
                        isStuck(0))
                    {
                        api.AutoFollow.IsAutoFollowing = false;
        ...
        {
            var exists = Directory.Exists(Application.StartupPath + @"\nav");
            if (!exists) Directory.CreateDirectory(Application.StartupPath + @"\nav");
            ...
            switch (saveFile.ShowDialog())
            {
                case DialogResult.OK:
                    ...
                    {
                        foreach (string waypoint in WayPoints.Items) file.WriteLine(waypoint);
                    }
            ...
            using (var sr = file.OpenText())
            {
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    if (items[0] == "WAYPOINT")
                    {
                        ...
                        navPathpause[ipos] = "";
                        if (items.Length > 4)
                            for (var i = 4; i <= items.Length - 1; i++)
                            {
                                if (items[i] == "First")
        ...
        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (navi.Exists) OpenNavi(navi.ToString());
        }
        ...
        private void RefreshToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            foreach (var file in Directory.GetFiles(path, "*.xin"))
                if (!comboBox2.Items.Contains(new FileInfo(file).Name))
                    comboBox2.Items.Add(new FileInfo(file).Name);
        }
        ...
        private void ConvertToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            for (var i = 0; i <= WayPoints.Items.Count - 1; i++)
            {
        ...
        private void Door_Click(object sender, EventArgs e)
        {
            ...
            if (index == -1)
            {
                if (MainWindow.TESTMODE && MainWindow.STATUS.Contains(@":: Final Fantasy Not Found ::"))
                    ...
                    WayPoints.Items.Add(
                        $"WAYPOINT:{ScriptFarmDNC.PlayerInfo.X}:{ScriptFarmDNC.PlayerInfo.Z}:{ScriptFarmDNC.PlayerInfo.Y}:Door;{ScriptFarmDNC.TargetInfo.ID}");
            }
            else
            {
                var old = (string) WayPoints.Items[index];
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (index == -1)
            {
                if (MainWindow.TESTMODE && MainWindow.STATUS == @":: Final Fantasy Not Found ::")
                    ...
                    WayPoints.Items.Add(
                        $"WAYPOINT:{ScriptFarmDNC.PlayerInfo.X}:{ScriptFarmDNC.PlayerInfo.Z}:{ScriptFarmDNC.PlayerInfo.Y}:Pause;{Pauseseconds.Value}");
            }
            else
            {
                var old = (string) WayPoints.Items[index];
