    ...
    public partial class ScriptSkillup : UserControl
    {
        ...
        private void loadMAsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            for (uint mm = 1; mm <= 895; mm++)
            {
                ...
                if (spellm == null || ScriptFarmDNC.skipSpellList.ContainsKey(mm) ||
                    playerMA.Items.Contains(spellm.Name[0])) continue;
                if (spelllvlm != -1)
                {
                    ...
                    else if (ScriptFarmDNC.PlayerInfo.MainJobLevel == 99 &&
                             ScriptFarmDNC.PlayerInfo.UsedJobPoints >= spelllvlm)
                    {
                        playerMA.Items.Add(spellm.Name[0]);
                    }
                }
            ...
            for (uint sm = 1; sm <= 895; sm++)
            {
                ...
                if (spells == null || ScriptFarmDNC.skipSpellList.ContainsKey(sm) ||
                    playerMA.Items.Contains(spells.Name[0])) continue;
                if (ScriptFarmDNC.PlayerInfo.HasSpell(sm) && ScriptFarmDNC.PlayerInfo.SubJobLevel >= spelllvls &&
                    spelllvls != -1)
                {
